...
Design_V = zeros(Size_V - o+1,o);
 ...
 for i = o  : Size_X; 
    ...
    for j = 1:o
       ...
       b = iteration;
       Para = zeros(1,o)';
        for iteration = 45401 : size(red_x,1)
           Var = Design_X(iteration,:)*Design_X(iteration,:)';
       ...
       errormat(45001:(iteration),:) = ((red_v(45001:(iteration),:)-(Design_X(45001:(iteration),:)*Para)));
     %errormat(1:size(red_x,1),:) = ((red_v(1:size(red_x,1),:)-(Design_X(1:size(red_x,1),:)*Para)));

% figure;   % Plotting Learning curve
%  plot(cost_matrix(:,1),cost_matrix(:,2));
%  xlabel('Iterations');
% ylabel('Mean Square Error');
% title('Learning curve ( a = 0.001 , b = 0.001 )');
% 
plot(errormat)
xlabel('Input');
ylabel('Error ( d(n)-y(n)');
title('Output for filter order 40 and a = 0.5');
